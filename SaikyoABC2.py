...
class SaikyoABC(object):
    ...
    def __draw_image(self,image):
        ...
        for s,p,w in zip(self.stroke.STROKE,self.stroke.PENTYPE,self.stroke.PENWIDTH):
            if p == "STYLUS":
                ...
                draw.text((np.min(self.stroke.STROKE[stroke_num][0]),
                           np.min(self.stroke.STROKE[stroke_num][1])),
                          str(stroke_num) ,fill=(0,0,255,128),font = font)
            elif p == "ERASEPEN":
    ...
    def draw_polygon_image(self,image_size=None):
        u"""ポリゴンを画像として出力する。
        -----
        input
            - image_size: 画像サイズ。(幅,高さ)で指定してください。※numpyの行列とは順番が逆なので注意！！
        ...
        if image_size is None:
            image_size = np.max(np.array([np.max(s,axis=1) for s in data.stroke.STROKE]),axis=0)
        image = Image.new("RGBA",image_size,color=(255,255,255,0))
        self.__draw_polygon_image(image)
        return image
    
    def overlay_polygon_image(self,originalimage):
        background = originalimage.copy()
        background = background.convert("RGBA")
        foreground = self.draw_polygon_image(background.size)
        background.paste(foreground,(0,0),foreground)
        return background
    
    def __draw_polygon_image(self,image):
        u"""
        ポリゴンのスケールを画像にプロットします
        """
        draw = ImageDraw.Draw(image)
        X,Y = 0,1
        xmin,xmax,ymin,ymax = 0,1,2,3
        font = ImageFont.truetype("arial.ttf", 32)
        for block_num in range(len(self.block_polygon)):
            point = self.__expand_polygon(self.block_polygon[block_num])
            hull = ConvexHull(point)
            for simplex in hull.simplices:
                draw.line((point[simplex[0]][0],point[simplex[0]][1],
                           point[simplex[1]][0],point[simplex[1]][1]),fill=(0,255,0),width = 1)
            draw.text(point.min(0), str(block_num) ,fill='#000',font = font)         
    ...
    def __include_stroke_polygon(self,block_num,stroke_num):
        ...
        for coo_num in range(len(self.stroke.STROKE[stroke_num][X])):
            self.block_polygon[block_num] = np.append(self.block_polygon[block_num],
                                                      np.array([[self.stroke.STROKE[stroke_num][X][coo_num],
                                                                 self.stroke.STROKE[stroke_num][Y][coo_num]]]),
                                                      axis = 0)
        
    #バグがあるかもしれません。out_vecの方向が逆になっているので引いています。
    #見た目には現状問題はありません。
    def __expand_polygon(self,polygon):
        u"""特定のブロックのポリゴンの領域を広げます。
        各ポリゴンの点をポリゴンの外側方向の角度の二等分線を引き、その点のポリゴンの中心点との距離による
        引き数polygonはself.block_polygon[block_num]です。"""
        hull = ConvexHull(polygon)
        center = np.array([0.0,0.0])
        for i in hull.vertices:
            #center = np.average(polygon,axis = 0)
            center += polygon[i]
        center /= len(hull.vertices)
        distances = np.array([np.linalg.norm(polygon[j]-center) for j in hull.vertices])
        dis_max = np.max(distances)
        dis_ave = np.average(distances)
        dis_sum = np.sum(distances)
        weight1 = 20
        weight2 = 50
        expanded_poly = np.empty((0,2))
        for poly_num in range(len(hull.vertices)): 
            pre_coo = hull.vertices[poly_num-2]
            cur_coo = hull.vertices[poly_num-1]
            pos_coo = hull.vertices[poly_num]
            pre_vec = polygon[pre_coo] - polygon[cur_coo]
            pos_vec = polygon[pos_coo] - polygon[cur_coo]
            out_vec1 = pre_vec/np.linalg.norm(pre_vec) + pos_vec/np.linalg.norm(pos_vec)
            u"""ペンストロークのブロックを距離におおじて少し膨らませるベクトル"""
            out_vec2 = polygon[cur_coo] - center
            u"""ペンストロークのブロックの内、突出した箇所をさらに遠くに伸ばすためのベクトル"""
            out_vec1 = out_vec1/np.linalg.norm(out_vec1)
            out_vec2 = out_vec2/np.linalg.norm(out_vec2)
            distance = np.linalg.norm(polygon[cur_coo]-center)
            expand_vec1 = -out_vec1*(distance/dis_max)**2*weight1
            expand_vec2 = out_vec2*(distance/dis_ave)*weight2
            expanded_poly = np.append(expanded_poly,[polygon[cur_coo]+expand_vec1+expand_vec2],axis = 0)
        expanded_poly = expanded_poly.astype(np.int64)
        return expanded_poly
    
    def check_include_polygon(self,point,polygon):
        u"""pointがpolygonの点群で構成される多角形に内包されるかを判定します。
        内包されるならTrue、されないならばFalseを返します。"""
        angle = 0
        for poly_num in range(len(polygon)):
            vec1 = polygon[poly_num]-point
            vec2 = polygon[poly_num-1]-point
    ...
    def __check_included(self,block_num,stroke_num):
        ...
        print(d4)
        """
        if len(self.block_polygon[block_num]) >= 3:
            poly_point = self.__expand_polygon(self.block_polygon[block_num])
            in_num = 0
            for coo_num in range(len(stroke[X])):
                point = np.array([stroke[X][coo_num],stroke[Y][coo_num]])
                if self.check_include_polygon(point,poly_point) == True:
                    in_num +=1
            if (in_num/len(stroke[X])) >= (40/100):
                return 1
    ...
    def stroke_classifier(self):
        ...
        self.block_polygon = [np.empty((0,2))]
        u"""
        """
        block_num = 0
        ...
        for stroke_num in range(len(self.stroke)) :
            if self.stroke.STROKE[stroke_num][0][0] == -1:
                ...
                self.__include_stroke_polygon(block_num,stroke_num)
            else:
                if self.__check_included(block_num,stroke_num) == 0:
                    ...
                    self.block_polygon.append(np.empty((0,2)))
                    block_num += 1
                    self.__include_stroke_polygon(block_num,stroke_num)
                else:
    ...
    def check_stroke_cross(self,stroke_num1,coo_num1,stroke_num2,coo_num2):
        ...
        交差するならTrue,交差しないならFalseを返します。
        coo_numは対応するストロークのcoo_num番目の点から交差の判定を始めます。"""
        X,Y = 0,1
...
overlayimage3 = abc.overlay_polygon_image(overlayimage)
overlayimage3
